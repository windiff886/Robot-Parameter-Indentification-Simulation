...
本文档详细介绍了在 BIRDy 框架及本项目当前的 **MuJoCo + 纯 C++** 实现中，如何评估机器人动力学参数辨识算法的性能与准确性。评估方法根据工作模式（仿真 vs 实验）而有所不同。
...
在当前实现中，`identify` 工具默认将数据按 **80% 训练 / 20% 验证** 划分，并直接输出每种算法的 Torque RMSE 与最大绝对误差。
